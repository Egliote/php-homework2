...
class YahooWP implements iWeatherProvider
{
    ...
    private function fetchCurrentTemperature(Location $location): float
    {
        ...
        if (!function_exists('curl_version')) {
            throw new WPException('curl is disabled. Install and/or enable it');
        }
        $curl = curl_init();
        ...
        if (curl_error($curl)) {
            ...
            curl_close($curl);
            throw new WPException(sprintf('curl error: %s', $curl_err));
        }
        curl_close($curl);
        $json = json_decode($json_string);

        if (null === $json) {
            throw new WPException('Can\'t get JSON from yahoo');
        }
    ...
    private function getTemperatureValue($json): float
    {
        if (isset($json->query->results) && (null === $json->query->results)) {
            ...
            throw new WPException('Yahoo didn\'t returned required data');
        }
        ...
        if (isset($json->query->results->channel->item->condition->temp)) {
            ...
            throw new WPException('Can\'t get temperature from yahoo');
        }
        return $temperature;
    }
}